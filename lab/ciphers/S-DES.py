...
def encrypt_simp_des(plain:str,kee:str)->str:
    ...
    print(f'after round 2 we have this before applying xor for the final step and swap too : {r2_out}')
...
def decrypt_simp_des(cipher:str,kee:str)->str:
    # there is a reuse of IP and P10 and 
    # all that so def them as main but too much work copy paste for 
    # 2 sub keys genrate first 

    # key generation inside again 
    P10 = [3,5,2,7,4,10,1,9,8,6] # positions to permutate into 
    p10 = ''
    for i in P10:
        p10 += kee[i-1] # indexing from 
    ...
    for i in IP:
        ct_IP += cipher[i-1]
    # round 2 begins reverse here so swithc the keys 
    
    L0 = ct_IP[:4] # left most 4 bits
    R0 = ct_IP[4:] # Right half 4 bits
    print("Round 1 begins : ")
    r1_out = fk(L0,R0,k2)
    print(f'output of round 1 {L0} {r1_out}')
    # r1 ends
    ...
    print('Round 2 begins ')
    print(f'now for round 2 we get the L1 : {L1} and R1 : {R1}')
    r2_out = fk(L1,R1,k1) # this time around we send key 2 as input for the xor layer
    print(f'after round 2 we have this before applying xor for the final step and swap too : {r2_out}')
    final_ip_inv_input = '' # first half we fill 
    for i in range(len(r2_out)):
        final_ip_inv_input += str(int(L1[i]) ^ int(r2_out[i])) 
    ...
    IP_inv = [4,1,3,5,7,2,8,6]
    for i in IP_inv:
        plain += final_ip_inv_input[i-1]
    
    return plain

# more eff if u made the key generation of 2 separately simpler to send / share between the client and server
...
cip = encrypt_simp_des(pt,ker)

print(decrypt_simp_des(cip,ker))
