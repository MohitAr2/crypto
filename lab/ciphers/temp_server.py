...
def decrypt_simp_des(cipher:str,kee:str)->str:
    ...
    for i in IP:
        ct_IP += cipher[i-1]
    print(f'IP in server side : {ct_IP}')
    ...
    r1_out = fk(L0,R0,k2)
    print(f'output of round 1 [insert sub-key 2 ] : {L0} {r1_out}')
    # r1 ends
    ...
    print(f'output after round 2 [insert sub-key 1 ] : {L1} {r2_out}')
...
def start_server():
    ...
    while True:
        ...
        if not message:
            break
        print(f"Client sent encrypted: {message}")#11010101
        fin = decrypt_simp_des(message,k)
